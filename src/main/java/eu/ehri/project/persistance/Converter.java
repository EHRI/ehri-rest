...
/**
 * Class containing static methods to convert between
 * FramedVertex instances, EntityBundles, and raw data.
 * 
 * @author michaelb
 *
 */
public class Converter {
     ...
     */
    public <T extends VertexFrame> EntityBundle<T> jsonToBundle(String json) throws DeserializationError {
     ...
     */
    public <T extends VertexFrame> String vertexFrameToJson(VertexFrame item) throws SerializationError {
     ...
     * 
     */
    public <T extends VertexFrame> String bundleToJson(EntityBundle<T> bundle) throws SerializationError {
     ...
     * 
     * Prize to whomever can remove all the unchecked warnings.
     * I don't really know how else to do this otherwise.
     * 
     * @throws DeserializationError 
     */
    @SuppressWarnings("unchecked")
    public <T extends VertexFrame> EntityBundle<T> dataToBundle(Map<String,Object> data) throws DeserializationError {
        try {
            ...
            if (isa == null)
                throw new DeserializationError(String.format("No '%s' attribute found in item data", EntityType.KEY));
            Class<T> cls = (Class<T>) classes.get(isa);
            if (cls == null)
                throw new DeserializationError(String.format("No class found for type %s type: '%s'", EntityType.KEY, isa));            
            MultiValueMap relationbundles = new MultiValueMap();

            Map<String,List<Map<String,Object>>> relations = (Map<String, List<Map<String, Object>>>) data.get("relationships");
            if (relations != null) {
                for (Entry<String,List<Map<String,Object>>> entry: relations.entrySet()) {
    ...
    public Map<String,Object> bundleToData(EntityBundle<?> bundle) {        
        ...
        for (Object key: bundle.getRelations().keySet()) {
            ...
            @SuppressWarnings("unchecked")
            Collection<EntityBundle<?>> collection = bundle.getRelations().getCollection(key);
     ...
     */
    @SuppressWarnings("unchecked")
    public <T extends VertexFrame> EntityBundle<T> vertexFrameToBundle(VertexFrame item) throws SerializationError {
        String isa = (String) item.asVertex().getProperty(EntityType.KEY);
        if (isa == null)
            throw new SerializationError(String.format("No %s key found in Vertex Properties to denote its type.", EntityType.KEY));
        Class<? extends VertexFrame> cls = classes.get(isa);
        if (cls == null)
            throw new SerializationError(String.format("No entity found for %s type '%s'", EntityType.KEY, isa));
            ...
            try {
                ...
                if (result instanceof Iterable<?>) {
                    for (Object d : (Iterable<?>)result) {
                        relations.put(entry.getKey(), vertexFrameToBundle((VertexFrame) d));
                    }
                } else {
                    // This relationship could be NULL if, e.g. a collection has no holder.
                    if (result != null)
                        relations.put(entry.getKey(), vertexFrameToBundle((VertexFrame)result));
                }
            } catch (Exception e) {
                throw new RuntimeException("Unexpected error serializing VertexFrame", e);
            }
        }
    ...
    private Map<String, Class<? extends VertexFrame>> getEntityClasses() {
        ...
        for (Class<?> cls : classArray) {
            if (VertexFrame.class.isAssignableFrom(cls)) {
                // NB: This is the unchecked cast, but it should be safe due to the
