...
public class AnnotationTest extends ModelTestBase {
    ...
    public void testUserHasAnnotation() {
        ...
        assertEquals(mike.getAnnotations().iterator().next().getBody(), TEST_ANNOTATION_BODY);
    }
    ...
    public void testUserHasAnnotationWithTarget() {
        ...
        // Check target
        assertEquals(annotation.getTarget().asVertex().getId(), c1.asVertex().getId());
    ...
    public void testAnnotationAnnotation() {
        AnnotatableEntity ann1 = helper.getTestFrame("ann1", AnnotatableEntity.class);
        Annotation ann2 = helper.getTestFrame("ann2", Annotation.class);
        
        assertEquals(ann2.getTarget(), ann1);        
        assertEquals(ann1.getAnnotations().iterator().next().getBody(), ann2.getBody());
    }
}
