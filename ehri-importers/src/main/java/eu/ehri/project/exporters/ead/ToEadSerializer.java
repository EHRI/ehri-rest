...
public class ToEadSerializer implements Visitor {
    ...
    protected void printImageTag(SuperNode imageNode, String url) {
        printer.print("<extref xlink:href=\"").print(url).print("\"  title=\"")
                .printEncoded(printChildrenToString(imageNode)).print("\"/>");
    }

    protected void printLink(LinkRenderer.Rendering rendering) {
        printer.print('<').print("extref");
        printAttribute("xlink:href", FastEncoder.encode(rendering.href));
        for (LinkRenderer.Attribute attr : rendering.attributes) {
            printAttribute(attr.name, attr.value);
        }
        printer.print('>').printEncoded(rendering.text).print("</extref>");
    }
